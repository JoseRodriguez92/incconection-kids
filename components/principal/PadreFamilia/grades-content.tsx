...
export function GradesContent() {
      ...
      try {
        ...
        const periodCycles: PeriodCycle[] = (apCycles ?? [])
          ...
          .sort((a: PeriodCycle, b: PeriodCycle) =>
            a.cycleName.localeCompare(b.cycleName, undefined, {
              numeric: true,
            }),
          );
  ...
  const startTour = useCallback(async () => {
    ...
    const steps: any[] = [
      {
        element: "#tour-grades-header",
        popover: {
          title: "📋 Registro de Notas",
          description:
            "Aquí puedes ver todas las calificaciones de tu hijo/a en el período académico activo, organizadas por materia y trimestre.",
          side: "bottom",
          align: "start",
        },
      },
    ...
    if (hasData) {
      steps.push(
        {
          element: "#tour-grades-stats",
          popover: {
            title: "📊 Resumen general",
            description:
              "Estas tarjetas muestran el total de materias, el promedio final del período, cuántas materias están aprobadas (≥ 84) y cuántas reprobadas (< 84).",
            side: "bottom",
          },
        },
        {
          element: "#tour-grades-tabla",
          popover: {
            title: "📚 Calificaciones por materia",
            description:
              "Cada fila es una materia. Las columnas muestran la nota de cada trimestre (ciclo) y la nota final generada por el sistema al cierre de cada ciclo.",
            side: "top",
          },
        },
        {
          element: "#tour-grades-leyenda",
          popover: {
            title: "🎨 Escala de desempeño",
            description:
              "SUPERIOR (107–120) · ALTO (95–106) · BÁSICO (84–94) · BAJO (< 84). Los colores de las notas siguen esta misma escala para identificar el nivel de un vistazo.",
            side: "top",
          },
        },
      );
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      {/* Header */}
      <div id="tour-grades-header" className="space-y-1 relative z-1">
        <div className="flex items-start justify-between gap-4">
          <h1 className="text-3xl font-bold tracking-tight">Registro de Notas</h1>
          <Button
            id="tour-grades-boton"
            variant="outline"
            size="sm"
      ...
      {!loading && !error && subjects.length > 0 && (
        <>
          {/* Tarjetas resumen */}
          <div id="tour-grades-stats" className="grid gap-4 md:grid-cols-4">
            <Card>
          ...
          <Card id="tour-grades-tabla">
            ...
            <CardContent>
              ...
              {/* Leyenda desempeño */}
              <div id="tour-grades-leyenda" className="flex items-center gap-3 flex-wrap mt-5 pt-4 border-t">
                ...
                {[
                  {
                    label: "SUPERIOR",
                    range: "107–120",
                    color: desempeñoColor("SUPERIOR"),
                  },
                  {
                    label: "ALTO",
                    range: "95–106",
                    color: desempeñoColor("ALTO"),
                  },
                  {
                    label: "BÁSICO",
                    range: "84–94",
                    color: desempeñoColor("BÁSICO"),
                  },
                  {
                    label: "BAJO",
                    range: "< 84",
                    color: desempeñoColor("BAJO"),
                  },
                ].map((d) => (
                  <Badge
                    key={d.label}
