...
import type { GroupWithStudents, SubjectReport, CycleGrade, LearningCondition } from "../types";

export function useResultadosData(selectedCourseId: string | null) {
  const [groups, setGroups] = useState<GroupWithStudents[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [fetchTick, setFetchTick] = useState(0);

  // Stores (cursos y períodos ya cacheados)
  ...
  // Período activo derivado del store
  const activePeriod = periodos.find((p) => p.is_active === true) ?? null;

  useEffect(() => {
    if (!selectedCourseId || !activePeriod) {
      setGroups([]);
      return;
    }
    ...
    const load = async () => {
      ...
      try {
        ...
        // ── 1. Grupos del curso dentro del período activo ─────────────────
        ...
        const parsedRaw: RawGroup[] = (groupsData ?? []).map((g) => {
          ...
          return {
            ...
            name: g.name,
            year: g.year,
            course: course ? { id: (course as any).id, name: (course as any).name } : null,
            students,
          };
        });
        ...
        const allEnrolledIds = parsedRaw.flatMap((g) => g.students.map((s) => s.enrolledId));

        // ── 2. Materias por grupo ─────────────────────────────────────────
        ...
        const cyclePeriodMap = new Map<string, string>();
        {
          ...
          for (const apc of apCyclesData ?? []) {
            ...
            if (!cycle) continue;
            cycleInfoMap.set(apc.id, { name: (cycle as any).name, startDate: apc.start_date });
            if (apc.academic_period_id) cyclePeriodMap.set(apc.id, apc.academic_period_id as string);
          }
        }
        ...
        if (allEnrolledIds.length > 0) {
          const { data: gradesData } = await supabase
            ...
            .select("grade, student_enrolled_id, group_has_class_id, academic_period_has_cycle_id")
          ...
          for (const row of gradesData ?? []) {
            ...
            byGhc.get(row.group_has_class_id)!.set(row.academic_period_has_cycle_id, row.grade);
          }
        }
        ...
        if (allStudentIds.length > 0) {
          ...
          for (const row of lcData ?? []) {
            const condition = Array.isArray(row.condition) ? row.condition[0] : row.condition;
    ...
    load();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedCourseId, activePeriod?.id, fetchTick]);
